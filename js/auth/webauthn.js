...
const BIOMETRIC_LAST_UID_KEY = 'pc_last_biometric_uid';
...
async function registrarHuella() {
  ...
  try {
    ...
    localStorage.setItem(BIOMETRIC_LAST_UID_KEY, _fbUser.uid);
...
async function loginConHuella() {
  ...
  const emailSolicitado = normalizarEmailBiometrico(emailInput?.value);
  ...
  let seleccionada;
  if (emailSolicitado) {
    seleccionada = creds.find(item => normalizarEmailBiometrico(item.email) === emailSolicitado);
    if (!seleccionada || emailSolicitado !== emailFirebase) {
      toast('La biometría registrada corresponde a otra cuenta.');
      return;
    }
  } else {
    const ultimoUid = localStorage.getItem(BIOMETRIC_LAST_UID_KEY);
    seleccionada = creds.find(item => item.uid === ultimoUid);
    if (!seleccionada) {
      toast('Ingresá el email de la cuenta biométrica para continuar.');
      return;
    }
    if (seleccionada.email && emailInput) emailInput.value = seleccionada.email;
    toast(`Biometría de ${seleccionada.email || seleccionada.username}`);
  }

  if (seleccionada.uid !== _fbUser.uid) {
    toast('La biometría registrada corresponde a otra cuenta.');
    return;
  }
  ...
  try {
    ...
    localStorage.setItem(BIOMETRIC_LAST_UID_KEY, matched.uid);
...
function eliminarHuella() {
  ...
  credencialesBiometricasGuardadas()
    .filter(item => item.uid === _fbUser.uid)
    .forEach(item => localStorage.removeItem(item.key));
  if (localStorage.getItem(BIOMETRIC_LAST_UID_KEY) === _fbUser.uid) {
    localStorage.removeItem(BIOMETRIC_LAST_UID_KEY);
  }
